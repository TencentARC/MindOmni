...
class OmniGenPipeline:
    ...
    def __call__(
        ...
        llm_user_input: Union[str, List[str]] = None,
        input_images: Union[List[str], List[List[str]]] = None,
        input_llm_images: Union[List[str], List[List[str]]] = None,
        height: int = 1024,
        width: int = 1024,
        ...
        input_data = self.processor(prompt, input_images, height=height, width=width, use_img_cfg=use_img_guidance, separate_cfg_input=separate_cfg_infer, use_input_image_size_as_output=use_input_image_size_as_output,
                                    llm_type=llm_type, llm_processor=self.llm_processor, input_llm_images=input_llm_images, llm_user_input=llm_user_input, pure_text=pure_text, use_edit_processor=use_edit_processor)
        ...
        if use_qwen:
            assert self.llm_processor is not None
            llm_input_embeds = []
            with torch.no_grad():
                # for seperate cfg infer mode
                for i in range(len(input_data['llm_padded_input_ids'])):
                    pixel_values = None
                    image_grid_thw = None
                    # import ipdb; ipdb.set_trace()
                    # TODO: only support batchsize=1 when input image to qwen
                    # assert len(input_data['llm_inputs'][i]) == 1, 'only support batchsize=1 when input image to qwen'
                    if input_data['llm_inputs'][i][0] is not None:
                        pixel_values = torch.tensor(input_data['llm_inputs'][i][0]['pixel_values']).to(self.device)
                        image_grid_thw = torch.tensor(input_data['llm_inputs'][i][0]['image_grid_thw']).to(self.device)

                    llm_inputs = dict(input_ids=input_data['llm_padded_input_ids'][i].to(self.device),
                                      attention_mask=input_data['llm_vae_attention_mask'][i].to(self.device),
                                      pixel_values=pixel_values,
                                      image_grid_thw=image_grid_thw,
                                      output_hidden_states=True)
                    llm_outputs = self.model_llm(**llm_inputs)

                    hidden_states = self.model.qwen2phi[0](llm_outputs.hidden_states[-1])
                    cache_position = torch.arange(0, hidden_states.shape[1], device=hidden_states.device)

                    # import ipdb; ipdb.set_trace()
                    try:   # llm lora case
                        mask_func = self.model_llm.base_model.model.model._update_causal_mask
                    except Exception as e:
                        mask_func = self.model_llm.model._update_causal_mask
                    cond_causal_mask = mask_func(
                        input_data['llm_vae_attention_mask'][i].to(self.device), hidden_states, cache_position, None, None)
                    for decoder_layer in self.model.qwen2phi[1:]:
                        layer_out = decoder_layer(
                            hidden_states,
                            attention_mask=cond_causal_mask,
                            position_ids=input_data['llm_vae_position_ids'][i].to(self.device),
                        )
            ...
            model_kwargs['llm_padded_input_ids'] = self.move_to_device(input_data['llm_padded_input_ids'])
            model_kwargs['llm_image_sizes'] = input_data['llm_image_sizes']
