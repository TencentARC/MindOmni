...
class ShortLongWebDataset(torch.utils.data.Dataset):
    ...
    def get_data_info(self, index: int) -> dict:
        ...
        image, byte_data = next(self.dataset)
        json_dict = json.loads(byte_data.decode('utf-8'))
        image = Image.open(io.BytesIO(image)).convert('RGB')

        # import ipdb; ipdb.set_trace()
        instruction = json_dict['qwen_caption']
        short_instruction = json_dict['metadata']['top_caption']

        if random.random() < self.condition_dropout_prob:
            instruction = '<cfg>'
        input_images = None

        mllm_input = self.processer.process_multi_modal_prompt(instruction, input_images, llm_type=self.llm_type, llm_processor=self.llm_processor, short_instruction=short_instruction)
...
if __name__ == "__main__":
    ...
    dataset = ShortLongWebDataset(
        ...
        # data_root='/group/40121/public_datasets/X2I_DATA/X2I-text-to-image/laion-coco-aesthetic_webdataset',
        # data_root='/group/40005/public_datasets/journeyDB/tars',
