...
from pretrain import OmniGenPipeline
...
from vlm_modules import compute_gen_kl_loss
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        val_iter: int = 1,
        diff_beta: float = 0.01,
        qwen2phi_module_origin_path: str = None,
        **kwargs,
    ):
        # Args
        if args is None:
        ...
        # generate func
        pipe = OmniGenPipeline.from_pretrained("/group/40043/yichengxiao/huggingface/model/OmniGen-v1")

        base_pretrained_path = '/'.join(qwen2phi_module_origin_path.split('/')[:-1])
        ...
        if os.path.exists(peft_pretrained_json):
            pipe.model = PeftModel.from_pretrained(pipe.model.cpu(), base_pretrained_path)
            print("pipe model is loading by peft")

        qwen2phi_module_origin = torch.load(qwen2phi_module_origin_path, map_location='cpu')
        pipe.model.qwen2phi.load_state_dict(qwen2phi_module_origin, strict=True)
        pipe.model = pipe.model.merge_and_unload()

        print(f"successfully load {qwen2phi_module_origin_path}")
        pipe.model.eval()
        self.weight_dtype = torch.bfloat16
        pipe.model.to(self.weight_dtype)
        ...
        # additional_train_modules = {'qwen2phi'}
        ...
        # vit, vit_processor = _get_clip_model_and_processor("/group/40043/yichengxiao/huggingface/model/clip-vit-base-patch16")
        vit, vit_processor = _get_clip_model_and_processor("/group/40043/yichengxiao/huggingface/model/clip-vit-large-patch14-336")
        self.vit = vit
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, output_hidden_states=False, **custom_multimodal_inputs):
        ...
        model_output = model(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=output_hidden_states, **custom_multimodal_inputs)  # (B, L, V)
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        with torch.no_grad():
            ...
            if self.beta == 0.0:
                ...
                with self.accelerator.unwrap_model(model).disable_adapter():
                    ref_per_token_logps, ref_hidden_states = self._get_per_token_logps(
                        model, prompt_completion_ids, attention_mask, output_hidden_states=True, **multimodal_inputs
                    )
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                if reward_func.__name__ == 'generation_score_reward':
                    self.pipe.to(model.device)
                    self.pipe.llm_processor = self.processing_class
                    self.pipe.model_llm = model
                    reward_kwargs['generate_fuc'] = self.pipe
                    reward_kwargs['clip_score_fn'] = (self.vit.to(self.model.device), self.vit_processor)
                    output_reward_func, output_images, messages, gt_prompt = reward_func(prompts=prompts, completions=completions, **reward_kwargs)
                else:
    ...
    def valid(self, model, system_content):

        # reasoning generation
        user_input_list = [
            "The object most commonly admired during the Mid-Autumn Festival.",
            "The type of clock found in most public places during the 19th century.",
        ...
        for user_input in user_input_list:
            ...
            messages = [system_content] + prompt_content
            text = self.processing_class.tokenizer.apply_chat_template(
            ...
            with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                generated_ids = unwrapped_model.generate(**inputs, max_new_tokens=2048)
        ...
        # editing case
        edit_case = [
            {"prompt": "<img><|image_1|></img> Add a pair of sunglasses to the cat in the image.", "input_images": ['/group/40034/yichengxiao/OmniGen/img_sources/img2.png']},
            {"prompt": "<img><|image_1|></img> Find all windowpanes in the photo and outline them in blue.", "input_images": ['/group/40034/yichengxiao/OmniGen/img_sources/img17.png']},
        ]
        for item in edit_case:
            with torch.no_grad():
                self.pipe.to(model.device)
                self.pipe.llm_processor = self.processing_class
                self.pipe.model_llm = model
                images = self.pipe(
                    ...
                    input_llm_images=item['input_images'],
                    use_input_image_size_as_output=True,
                )[0]
                images_list.append(images)

        concatenated_image = Image.new('RGB', (images_list[0].width * len(images_list), images_list[0].height))
        for idx, img in enumerate(images_list):
            loc = idx * images_list[0].width
            concatenated_image.paste(img, (loc, 0))  # img1 放在左边

        base_dir = os.path.join(self.output_dir, 'valid_images')
        os.makedirs(base_dir, exist_ok=True)
        save_path = os.path.join(base_dir, f'{int(self.state.global_step):05d}.png')
        try:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        system_content = inputs[0]['prompt'][0]
        assert system_content['role'] == 'system'
        if self.val_iter != 0:
            if self.state.global_step % self.val_iter == 0:
                if self.accelerator.is_main_process:
                    print("validation beginning ......")
                    self.valid(model, system_content)
        if return_outputs:
        ...
        ref_hidden_states = inputs['ref_hidden_states']
        ...
        per_token_logps, hidden_states = self._get_per_token_logps(model, input_ids, attention_mask, output_hidden_states, **multimodal_inputs)
        # Get rid of the prompt (-1 because of the shift done in get_per_token_logps)
        per_token_logps = per_token_logps[:, prompt_ids.size(1) - 1:]

        # compute diffusion kl
        diffusion_kl_list = compute_gen_kl_loss(inputs['output_images'], inputs['gt_prompt'], inputs['messages'], self.pipe.vae, self.pipe.processor, self.processing_class, model, self.pipe.model, self.weight_dtype, hidden_states, attention_mask, input_ids, ref_hidden_states)
        ...
        # add diffusion kl if diff_beta > 0
        if self.diff_beta > 0:
            mean_diff_kl = torch.mean(torch.stack(diffusion_kl_list))
            loss = loss + mean_diff_kl * self.diff_beta

            # Log KL divergence
            self._metrics["diff_kl"].append(self.accelerator.gather_for_metrics(mean_diff_kl).mean().item())
